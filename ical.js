...
const typeParameter = function (name) {
    ...
    return function (value, parameters, curr) {
        const returnValue = isDateOnly(value, parameters)
            ? 'date'
            : 'date-time';
        ...
        for (const entry of dates) {
            ...
            if (exdate[name]) {
                if (typeof exdate[name].toISOString === 'function') {
                    curr[name][exdate[name].toISOString().slice(0, 10)] =
                        exdate[name];
                } else {
                    throw new TypeError(
...
module.exports = {
        ...
        END(value, parameters, curr, stack) {
            ...
            const originalEnd = function (component, parameters_, curr, stack) {
                ...
                if (!curr.end) {
                    ...
                    if (curr.datetype === 'date-time') {
                        ...
                        curr.end = moment
                            .utc(curr.start)
                            .add(1, 'days')
                            .toDate(); // New Date(moment(curr.start).add(1, 'days'));
                    } else {
                        const durationUnits = {
                            // Y: 'years',
                            // M: 'months',
                        ...
                        const indicator = curr.duration.startsWith('-')
                            ? -1
                            : 1;
                ...
                if (curr.uid) {
                    ...
                    if (typeof curr.recurrenceid !== 'undefined') {
                        ...
                        if (
                            typeof curr.recurrenceid.toISOString === 'function'
                        ) {
                            par[curr.uid].recurrences[
            ...
            if (
                ...
                if (rule.includes('DTSTART') === false) {
                    ...
                    let x = moment(curr.start).format(
                    ...
                    if (x.slice(-11) === '12:00:00 am') {
                        ...
                        if (offset < 0) {
                            ...
                            curr.start = new Date(
                                curr.start.getTime() + Math.abs(offset) * 60_000
                            );
                        } else {
    ...
    handleObject(name, value, parameters, ctx, stack, line) {
        if (this.objectHandlers[name]) {
            return this.objectHandlers[name](
                value,
                parameters,
                ctx,
                stack,
    ...
    parseLines(lines, limit, ctx, stack, lastIndex, cb) {
        ...
        for (let ii = lines.length; i < ii; i++) {
            ...
            ctx =
                this.handleObject(name, value, parameters, ctx, stack, l) || {};
