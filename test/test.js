    ...
    .addBatch({
                ...
                'event 47f6e': {
                    topic(events) {
                        return events.calendarComponents.find(
                            (x) =>
                                x.uid ===
                                '47f6ea3f28af2986a2192fa39a91fa7d60d26b76'
                        );
                    },
                ...
                'event 480a': {
                    topic(events) {
                        return events.calendarComponents.find(
                            (x) =>
                                x.uid ===
                                '480a3ad48af5ed8965241f14920f90524f533c18'
                        );
                    },
                    'has a summary (invalid colon handling tolerance)'(topic) {
                        assert.equal(
                            topic.summary,
                            '[Async]: Everything Express'
                        );
                    },
            ...
            'tzid parsing': {
                topic(events) {
                    return events.calendarComponents.find(
                        (object) =>
                            object.uid ===
                            'EC9439B1-FF65-11D6-9973-003065F99D04'
                    );
                },
            ...
            'event c32a5...': {
                topic(events) {
                    return events.calendarComponents.find(
                        (x) =>
                            x.uid ===
                            'c32a5eaba2354bb29e012ec18da827db90550a3b@tripit.com'
                    );
                },
                'has a start datetime'(topic) {
                    assert.equal(topic.start.getFullYear(), 2011);
                ...
                'has a summary'(topic) {
                    ...
                    assert.equal(
                        topic.summary,
                        'South San Francisco, CA, October 2011;'
                    );
                },
                ...
                'has a description'(topic) {
                    const desired =
                        ...
                        'View and/or edit details in TripIt : http://www.tripit.com/trip/show/id/23710889\n' +
        ...
        'with test10.ics': {
            ...
            'when categories present with trailing whitespace': {
                ...
                'should contain individual category values without whitespace'(
                    event
                ) {
                    assert.deepEqual(event.categories, [
                        'cat1',
        ...
        'with test11.ics (testing zimbra freebusy)': {
            ...
            'freebusy busy events': {
                topic(events) {
                    return events.calendarComponents
                        .find((y) => y.freebusy)
                        .freebusy.find((x) => x.type === 'BUSY');
                },
                'has an start datetime'(topic) {
                    assert.equal(topic.start.getFullYear(), 2014);
                    assert.equal(topic.start.getMonth(), 3);
        ...
        'with test14.ics (testing comma-separated exdates)': {
            ...
            'event with comma-separated exdate': {
                ...
                'Has summary "Example of comma-separated exdates"'(topic) {
                    assert.equal(
                        topic.summary,
                        'Example of comma-separated exdates'
                    );
                },
        ...
        'with test14.ics (testing exdates with bad times)': {
            ...
            'event with exdates with bad times': {
                ...
                'Has summary "Example of exdate with bad times"'(topic) {
                    assert.equal(
                        topic.summary,
                        'Example of exdate with bad times'
                    );
                },
                ...
                'event with start and end including timezones': {
                    ...
                    'has a start'(topic) {
                        ...
                        assert.equal(
                            ...
                            new Date(Date.UTC(2019, 3, 30, 9, 0, 0))
                                .toISOString()
                                .slice(0, 8)
                        );
                        assert.equal(
        ...
        'with Office-2012-owa.ics (testing for old ms timezones before DST)': {
            ...
            'event with old TZ': {
                ...
                'is not valid timezone'(topic) {
                    assert.equal(
                        topic.end.toISOString().slice(0, 8),
                        new Date(Date.UTC(2020, 9, 28, 15, 0, 0))
                            .toISOString()
                            .slice(0, 8)
                    );
                },
        ...
        'with Office-2012-owa.ics (testing for old ms timezones after DST )': {
            ...
            'event with old TZ': {
                ...
                'is not valid timezone'(topic) {
                    assert.equal(
                        topic.end.toISOString().slice(0, 8),
                        new Date(Date.UTC(2020, 10, 2, 20, 0, 0))
                            .toISOString()
                            .slice(0, 8)
                    );
                },
            ...
            'event with old TZ': {
                ...
                'is not valid timezone'(topic) {
                    assert.equal(
                        ...
                        new Date(Date.UTC(2021, 2, 25, 10, 35, 0))
                            .toISOString()
                            .slice(0, 8)
                    );
                },
        ...
        'bad rrule': {
            ...
            'is valid time': {
                ...
                'is not valid date'(topic) {
                    assert.equal(
                        topic.start.toISOString().slice(11),
                        '15:50:00.000Z'
                    );
                },
            },
