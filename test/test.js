...
// Vows no release can not be replaced ...
  ...
  .addBatch({
    'when parsing test1.ics (node conferences schedule from lanyrd.com, modified)':
      {
        ...
        'we get 9 events'(topic) {
          const events = topic.calendarComponents.filter(
            x => x.type === 'VEVENT',
          );
        ...
        'event 47f6e': {
          topic(events) {
            return events.calendarComponents.find(
              x => x.uid === '47f6ea3f28af2986a2192fa39a91fa7d60d26b76',
            );
          },
        ...
        'event 480a': {
          topic(events) {
            return events.calendarComponents.find(
              x => x.uid === '480a3ad48af5ed8965241f14920f90524f533c18',
            );
          },
        ...
        'event d4c8': {
          topic(events) {
            return events.calendarComponents.find(
              x => x.uid === 'd4c826dfb701f611416d69b4df81caf9ff80b03a',
            );
          },
    ...
    'with test2.ics (testing ical features)': {
      ...
      'todo item uid4@host1.com': {
        topic(items) {
          return items.calendarComponents.find(
            object => object.uid === 'uid4@host1.com',
          );
        },
        'is a VTODO'(topic) {
      ...
      'tzid parsing': {
        topic(events) {
          return events.calendarComponents.find(
            object => object.uid === 'EC9439B1-FF65-11D6-9973-003065F99D04',
          );
        },
    ...
    'with test3.ics (testing tvcountdown.com)': {
      ...
      'event -83': {
        topic(events) {
          return events.calendarComponents.find(
            x => x.uid === '20110505T220000Z-83@tvcountdown.com',
          );
        },
        'has a start datetime'(topic) {
    ...
    'with test5.ics (testing meetup.com)': {
      ...
      'event nsmxnyppbfc@meetup.com': {
        topic(events) {
          return events.calendarComponents.find(
            x => x.uid === 'event_nsmxnyppbfc@meetup.com',
          );
        },
        'has a start'(topic) {
    ...
    'with test6.ics (testing assembly.org)': {
      ...
      'event with no ID': {
        topic(events) {
          return events.calendarComponents.find(
            x => x.summary === 'foobar Summer 2011 starts!',
          );
        },
        'has a start'(topic) {
          assert.equal(
      ...
      'event with rrule': {
        topic(events) {
          return events.calendarComponents.find(
            x => x.summary === 'foobarTV broadcast starts',
          );
        },
        'Has an RRULE'(topic) {
    ...
    'with test11.ics (testing zimbra freebusy)': {
      ...
      'freebusy busy events': {
        topic(events) {
          return events.calendarComponents.find(y => y.freebusy).freebusy.find(x => x.type === 'BUSY');
        },
        'has an start datetime'(topic) {
          assert.equal(topic.start.getFullYear(), 2014);
          assert.equal(topic.start.getMonth(), 3);
    ...
    'with test13.ics (testing recurrence-id before rrule)': {
      ...
      'event with rrule': {
        topic(events) {
          return events.calendarComponents.find(
            x => x.uid === '6m2q7kb2l02798oagemrcgm6pk@google.com',
          );
        },
        'Has an RRULE'(topic) {
          assert.notEqual(topic.rrule, undefined);
    ...
    'with test14.ics (testing comma-separated exdates)': {
      ...
      'event with comma-separated exdate': {
        topic(events) {
          return events.calendarComponents.find(
            x => x.uid === '98765432-ABCD-DCBB-999A-987765432123',
          );
        },
    ...
    'with test14.ics (testing exdates with bad times)': {
      ...
      'event with exdates with bad times': {
        topic(events) {
          return events.calendarComponents.find(
            x => x.uid === '1234567-ABCD-ABCD-ABCD-123456789012',
          );
        },
    ...
    'with test18.ics (testing for detecting timezones)': {
      ...
      'event 1c943': {
        topic(events) {
          return events.calendarComponents.find(
            x => x.uid === '1C9439B1-FF65-11D6-9973-003065F99D04',
          );
        },
        'datetype is date-time'(topic) {
          assert.equal(topic.datetype, 'date-time');
      ...
      'event 2c943': {
        topic(events) {
          return events.calendarComponents.find(
            x => x.uid === '2C9439B1-FF65-11D6-9973-003065F99D04',
          );
        },
        'datetype is date-time'(topic) {
          assert.equal(topic.datetype, 'date-time');
      ...
      'event 3c943': {
        topic(events) {
          return events.calendarComponents.find(
            x => x.uid === '3C9439B1-FF65-11D6-9973-003065F99D04',
          );
        },
        'datetype is date-time'(topic) {
          assert.equal(topic.datetype, 'date-time');
      ...
      'event 4c943': {
        topic(events) {
          return events.calendarComponents.find(
            x => x.uid === '4C9439B1-FF65-11D6-9973-003065F99D04',
          );
        },
        'datetype is date'(topic) {
          assert.equal(topic.datetype, 'date');
      ...
      'event 5c943': {
        topic(events) {
          return events.calendarComponents.find(
            x => x.uid === '5C9439B1-FF65-11D6-9973-003065F99D04',
          );
        },
        'datetype is date'(topic) {
          assert.equal(topic.datetype, 'date');
    ...
    'with ms_timezones.ics (testing time conversions)': {
      ...
      'event with time in CET': {
        topic(events) {
          return events.calendarComponents.find(
            x => x.summary === 'Log Yesterday\'s Jira time',
          );
        },
