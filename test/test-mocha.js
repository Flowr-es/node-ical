...
describe('when parsing test1.ics (node conferences schedule from lanyrd.com, modified)', () => {
  const topic = ical.parseFile('./test/test1.ics');
  console.log(JSON.stringify(topic));
  const events = topic.calendarComponents.filter(x => x.type === 'VEVENT');

  it('we get 9 events', () => {
    assert.equal(events.length, 9);
  });

  const event1Test1 = topic.calendarComponents.find(x => x.uid === '47f6ea3f28af2986a2192fa39a91fa7d60d26b76');

  it('event 47f6e is in fort lauderdale', () => {
    assert.equal(event1Test1.location, 'Fort Lauderdale, United States');
  });

  it('event 47f6e starts Tue, 29 Nov 2011', () => {
    assert.equal(
      event1Test1.start.toDateString(),
      new Date(2011, 10, 29).toDateString(),
    );
  });

  it('event 47f6e datetype is date', () => {
    assert.equal(event1Test1.datetype, 'date');
  });

  it('event 47f6e method is PUBLISH', () => {
    assert.equal(event1Test1.method, 'PUBLISH');
  });

  //       'event 480a': {
  //         topic(events) {
  //           return events.calendarComponents.find(x => x.uid === '480a3ad48af5ed8965241f14920f90524f533c18');
  //         },
  ...
  //       'event d4c8': {
  //         topic(events) {
  //           return events.calendarComponents.find(x => x.uid === 'd4c826dfb701f611416d69b4df81caf9ff80b03a');
  //         },
  ...
  //         topic(events) {
  //           return events.calendarComponents.find(x => x.uid === 'sdfkf09fsd0');
  //         },
  //         'has a start datetime'(topic) {
...
//       topic(items) {
//         return items.calendarComponents.find(object => object.uid === 'uid4@host1.com');
//       },
//       'is a VTODO'(topic) {
...
//     vfreebusy: {
//       topic(events) {
//         return events.calendarComponents.find(x => x.type === 'VFREEBUSY');
//       },
//       'has a URL'(topic) {
//         assert.equal(
//           topic.url,
...
//       topic(events) {
//         return events.calendarComponents.find(x => x.type === 'VFREEBUSY').freebusy[0];
//       },
...
//     'tzid parsing': {
//       topic(events) {
//         return events.calendarComponents.find(object => object.uid === 'EC9439B1-FF65-11D6-9973-003065F99D04');
//       },
...
//     'event -83': {
//       topic(events) {
//         return events.calendarComponents.find(x => x.uid === '20110505T220000Z-83@tvcountdown.com');
//       },
//       'has a start datetime'(topic) {
...
//     'event c32a5...': {
//       topic(events) {
//         return events.calendarComponents.find(x => (
...
//       topic(events) {
//         return events.calendarComponents.find(x => x.uid === 'event_nsmxnyppbfc@meetup.com');
//       },
//       'has a start'(topic) {
//         assert.equal(topic.start.tz, 'America/Phoenix');
//         assert.equal(
//           topic.start.toISOString(),
//           new Date(Date.UTC(2011, 10, 10, 2, 0, 0)).toISOString(),
//         );
//       },
//       'method is PUBLISH'(topic) {
//         assert.equal(topic.method, 'PUBLISH');
...
//     'event with no ID': {
//       topic(events) {
//         return events.calendarComponents.find(x => x.summary === 'foobar Summer 2011 starts!');
//       },
//       'has a start'(topic) {
//         assert.equal(
//           topic.start.toISOString(),
//           new Date(2011, 7, 4, 0, 0, 0).toISOString(),
//         );
//       },
//     },
//     'event with rrule': {
//       topic(events) {
//         return events.calendarComponents.find(x => x.summary === 'foobarTV broadcast starts');
//       },
//       'Has an RRULE'(topic) {
...
//       topic(events) {
//         const ev = events.calendarComponents[0];
...
//     'grabbing VTODO task': {
//       topic(topic) {
//         return topic.calendarComponents[0];
//       },
//       'task completed'(task) {
//         assert.equal(task.completion, 100);
//         assert.equal(
//           task.completed.toISOString(),
//           new Date(2013, 6, 16, 10, 57, 45).toISOString(),
//         );
//       },
//     },
//   },
...
//     'freebusy params': {
//       topic(events) {
//         return events.calendarComponents[0];
//       },
//       'has a URL'(topic) {
//         assert.equal(
//           topic.url,
...
//       topic(events) {
//         return events.calendarComponents.map(y => y.freebusy)(x => x.type === 'BUSY')[0];
//       },
//       'has an start datetime'(topic) {
//         assert.equal(topic.start.getFullYear(), 2014);
//         assert.equal(topic.start.getMonth(), 3);
...
//     'event with rrule': {
//       topic(events) {
//         return events.calendarComponents.find(x => x.uid === '0000001');
//       },
//       'Has an RRULE'(topic) {
//         assert.notEqual(topic.rrule, undefined);
//       },
...
//         assert.notEqual(
//           topic.exdate[
//             new Date(Date.UTC(2015, 6, 8, 19, 0, 0))
//               .toISOString()
//               .slice(0, 10)
//           ],
//           undefined,
...
//         assert.notEqual(
//           topic.exdate[
//             new Date(Date.UTC(2015, 6, 10, 19, 0, 0))
//               .toISOString()
//               .slice(0, 10)
//           ],
//           undefined,
...
//         assert.notEqual(
//           topic.recurrences[
//             new Date(Date.UTC(2015, 6, 7, 19, 0, 0))
//               .toISOString()
//               .slice(0, 10)
//           ],
//           undefined,
...
//     'event with rrule': {
//       topic(events) {
//         return events.calendarComponents.find(x => x.uid === '6m2q7kb2l02798oagemrcgm6pk@google.com');
//       },
//       'Has an RRULE'(topic) {
//         assert.notEqual(topic.rrule, undefined);
//       },
...
//         assert.notEqual(
//           topic.recurrences[
//             new Date(Date.UTC(2016, 7, 26, 11, 0, 0))
//               .toISOString()
//               .slice(0, 10)
//           ],
//           undefined,
...
//       topic(events) {
//         return events.calendarComponents.find(x => x.uid === '98765432-ABCD-DCBB-999A-987765432123');
//       },
...
//         assert.notEqual(
//           topic.exdate[
//             new Date(2017, 6, 6, 12, 0, 0).toISOString().slice(0, 10)
//           ],
//           undefined,
//         );
//         assert.notEqual(
//           topic.exdate[
//             new Date(2017, 6, 17, 12, 0, 0).toISOString().slice(0, 10)
//           ],
//           undefined,
//         );
//         assert.notEqual(
//           topic.exdate[
//             new Date(2017, 6, 20, 12, 0, 0).toISOString().slice(0, 10)
//           ],
//           undefined,
//         );
//         assert.notEqual(
//           topic.exdate[
//             new Date(2017, 7, 3, 12, 0, 0).toISOString().slice(0, 10)
//           ],
//           undefined,
//         );
...
//         assert.equal(
//           topic.exdate[
//             new Date(2017, 4, 5, 12, 0, 0).toISOString().slice(0, 10)
//           ],
//           undefined,
//         );
//       },
...
//       topic(events) {
//         return events.calendarComponents.find(x => x.uid === '1234567-ABCD-ABCD-ABCD-123456789012');
//       },
...
//         assert.notEqual(
//           topic.exdate[
//             new Date(2017, 11, 18, 12, 0, 0).toISOString().slice(0, 10)
//           ],
//           undefined,
//         );
//         assert.notEqual(
//           topic.exdate[
//             new Date(2017, 11, 19, 12, 0, 0).toISOString().slice(0, 10)
//           ],
//           undefined,
//         );
//       },
...
//         assert.equal(
//           topic.end.toISOString().slice(0, 8),
//           new Date(Date.UTC(2019, 3, 30, 9, 0, 0)).toISOString().slice(0, 8),
//         );
//         assert.equal(topic.end.tz, '(UTC+07:00) Bangkok, Hanoi, Jakarta');
//         assert.equal(
//           topic.end.toISOString().slice(0, 8),
//           new Date(2019, 3, 30, 5, 0, 0).toISOString().slice(0, 8),
//         );
//       },
//     },
//   },
...
//     'event 1c943': {
//       topic(events) {
//         return events.calendarComponents.find(x => x.uid === '1C9439B1-FF65-11D6-9973-003065F99D04');
//       },
//       'datetype is date-time'(topic) {
//         assert.equal(topic.datetype, 'date-time');
//       },
...
//         assert.equal(
//           topic.start.toISOString(),
//           new Date(2002, 9, 28, 1, 20, 30).toISOString(),
//         );
//       },
...
//     'event 2c943': {
//       topic(events) {
//         return events.calendarComponents.find(x => x.uid === '2C9439B1-FF65-11D6-9973-003065F99D04');
//       },
//       'datetype is date-time'(topic) {
//         assert.equal(topic.datetype, 'date-time');
//       },
...
//     'event 3c943': {
//       topic(events) {
//         return events.calendarComponents.find(x => x.uid === '3C9439B1-FF65-11D6-9973-003065F99D04');
//       },
//       'datetype is date-time'(topic) {
//         assert.equal(topic.datetype, 'date-time');
//       },
...
//     'event 4c943': {
//       topic(events) {
//         return events.calendarComponents.find(x => x.uid === '4C9439B1-FF65-11D6-9973-003065F99D04');
//       },
...
//         assert.equal(
//           topic.start.toISOString(),
//           new Date(2002, 9, 28).toISOString(),
//         );
//       },
//       'method is REQUEST'(topic) {
//         assert.equal(topic.method, 'REQUEST');
...
//     'event 5c943': {
//       topic(events) {
//         return events.calendarComponents.find(x => x.uid === '5C9439B1-FF65-11D6-9973-003065F99D04');
//       },
//       'datetype is date'(topic) {
//         assert.equal(topic.datetype, 'date');
//       },
...
//       topic(events) {
//         return events.calendarComponents.find(x => x.summary === 'Log Yesterday\'s Jira time');
//       },
...
//     'event with bad TZ': {
//       topic(events) {
//         return events.calendarComponents.find(x => x.summary === '[private]');
//       },
...
//     'event with old TZ': {
//       topic(events) {
//         return events.calendarComponents.find(x => x.summary === ' TEST');
//       },
//       'is not valid timezone'(topic) {
//         assert.equal(
//           topic.end.toISOString().slice(0, 8),
//           new Date(Date.UTC(2020, 9, 28, 15, 0, 0)).toISOString().slice(0, 8),
//         );
//       },
//     },
//   },
...
//     'event with old TZ': {
//       topic(events) {
//         return events.calendarComponents.find(x => x.summary === ' TEST 3');
//       },
//       'is not valid timezone'(topic) {
//         assert.equal(
//           topic.end.toISOString().slice(0, 8),
//           new Date(Date.UTC(2020, 10, 2, 20, 0, 0)).toISOString().slice(0, 8),
//         );
//       },
//     },
//   },
...
//       'is not valid timezone'(topic) {
//         assert.equal(
//           topic.start.toISOString().slice(0, 8),
//           new Date(Date.UTC(2021, 2, 25, 10, 35, 0)).toISOString().slice(0, 8),
//         );
//       },
//     },
//   },
...
//       'event with old TZ': {
//         topic(events) {
//           return events.calendarComponents.find(x => x.summary === '*masked-away*');
//         },
//         'is not valid timezone'(topic) {
//           assert.equal(
//             topic.end.toISOString().slice(0, 8),
...
//       'event with old TZ': {
//         topic(events) {
//           return events.calendarComponents.find(x => x.summary === '*masked-away2*');
//         },
//         'is not valid timezone'(topic) {
//           assert.equal(
//             topic.end.toISOString().slice(0, 8),
//             new Date(Date.UTC(2021, 2, 23, 21, 56, 56))
//               .toISOString()
//               .slice(0, 8),
//           );
//         },
...
//     'is valid time': {
//       topic(events) {
//         return events.calendarComponents.find(x => x.summary === 'Academic Time');
//       },
...
//     'event with east TZ': {
//       topic(events) {
//         return events.calendarComponents.find(x => x.summary === 'Fear TWD');
//       },
//       'is not valid date'(topic) {
