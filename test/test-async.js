...
vows
  ...
  .addBatch({
    'when parsing test1.ics (node conferences schedule from lanyrd.com, modified)': {
      ...
      'event 47f6e': {
        topic(events) {
          return events.calendarComponents.find(x => x.uid === '47f6ea3f28af2986a2192fa39a91fa7d60d26b76');
        },
    ...
    'with test4.ics (testing tripit.com)': {
      ...
      'event c32a5...': {
        topic(events) {
          return events.calendarComponents.find(x => x.uid === 'c32a5eaba2354bb29e012ec18da827db90550a3b@tripit.com');
        },
        'has a start datetime'(topic) {
          assert.equal(topic.start.getFullYear(), 2011);
    ...
    'with test10.ics': {
      ...
      'when categories present with trailing whitespace': {
        topic(topic) {
          return topic.calendarComponents[1];
        },
      ...
      'when categories present but empty': {
        topic(topic) {
          return topic.calendarComponents[2];
        },
      ...
      'when categories present but singular': {
        topic(topic) {
          return topic.calendarComponents[3];
        },
      ...
      'when categories present on multiple lines': {
        topic(topic) {
          return topic.calendarComponents[4];
        },
    ...
    'with test11.ics (testing zimbra freebusy)': {
      ...
      'freebusy busy events': {
        topic(events) {
          return events.calendarComponents[0].freebusy.find(x => x.type === 'BUSY');
          // _.select(_.values(events)[0].freebusy, x => x.type === 'BUSY')[0];
        },
        'has an start datetime'(topic) {
          assert.equal(topic.start.getFullYear(), 2014);
          assert.equal(topic.start.getMonth(), 3);
